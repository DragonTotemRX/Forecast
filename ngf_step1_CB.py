...
while psutil.virtual_memory().percent>max_mem: 
    ...
    time.sleep(60)
os.system('{ python '+my_code_loc+'CB.py '+sys.argv[1]+' '+sys.argv[2]+' M1_P ; } > '+my_code_loc+'log/'+sys.argv[1]+'_cb_stocked.log 2>&1 &')
...
while psutil.virtual_memory().percent>max_mem: 
    ...
    print('mem pct='+str(psutil.virtual_memory().percent))
    time.sleep(60)
os.system('{ python '+my_code_loc+'CB.py '+sys.argv[1]+' '+sys.argv[2]+' M2_P ; } > '+my_code_loc+'log/'+sys.argv[1]+'_cb_full.log 2>&1 &')
print('clock='+time.strftime("%m/%d/%Y, %H:%M:%S", time.localtime()))
